...
- **Основний UI** (`index.html` + `app.js`) для FX переважно використовує **tick volume** (проксі-обсяг): `tick_count/ticks/intensity`.
  - Дефолт: `?fxcm_tickcount_volume=1` (можна вимкнути: `?fxcm_tickcount_volume=0`).
...
Правило відображення (наближено до TradingView):

- Висота бруска визначається **лише** числом `volume` у цьому барі.
- Масштаб (Y) для volume **підганяється під видимий діапазон часу** з невеликим запасом (headroom), без кліпінгу «в стелю».
...
## Інваріанти взаємодії з графіком (канонічна UX)

Це **очікувана** поведінка графіка (якщо відхиляється — це баг).

- Drag у основній області графіка (pane):
  - по X: панорамування по часу (вліво/вправо);
  - по Y: вертикальний зсув (vertical-pan) — можна підняти/опустити ціну в межах вікна.
- Drag по правій ціновій осі (price-scale):
  - змінює **масштаб по Y** (zoom/scale),
  - не має перетворюватись на time-pan/зсув «вбік».
- Wheel над правою ціновою віссю:
  - масштабує по Y (zoom по ціні),
  - не має рухати графік вліво/вправо.
- Wheel над pane:
  - працює як стандартний wheel для lightweight-charts (time-zoom/scroll за дефолтом UI).

Технічна примітка: наші додаткові взаємодії не повинні конфліктувати з нативним scale бібліотеки.

### Як “піймати” разовий стрибок/розмазування

Якщо стрибок проявляється на мить і потім зникає, вмикай debug-фіксацію:

- Відкрий UI з параметром `?debug_chart=1`.
- При появі аномалії debug-оверлей **заморозиться** і збереже дамп у:
  - `window.__chartDebugLastAnomaly` (у DevTools console)
  - `localStorage["smc_ui_debug_chart_last_anomaly"]`

Це дозволяє подивитись точні `manualRange/effectiveRange/price` вже після того, як графік «вирівнявся».

### Відкладено: інколи графік «завмирає»

Інколи під час звичайного руху/drag (вліво/вправо або по осі) графік може тимчасово “залипнути” (не реагує як очікується).

- Поточний статус: **не критично, відкладено**.
- `debug_chart=1` поки що **не відловлює** цей кейс стабільно.
- Повернутися до діагностики пізніше (коли буде репродакшн/дамп з DevTools).
