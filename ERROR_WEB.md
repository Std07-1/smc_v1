...
## Фаза 5.5 — Таймфрейми 1m/5m та яскраві BOS (2025-12-11)

- **Що додали:** у шапці поруч із селектором символів зʼявився селектор таймфрейму (`1m`/`5m`). Він зберігає вибір у `appState.currentTimeframe`, синхронізує значення у DOM і під час кожного переходу робить новий `fetchOhlcv` з відповідним параметром `tf`. Після `setBars` ми викликаємо `rehydrateOverlays()`, тож усі BOS/OTE/Zones миттєво перемальовуються у новому масштабі.
- **Захист від сміття:** `normalizeTimeframe()` приймає лише whitelisted значення, тому бекенд не отримає некоректний `tf`, а селектор завжди показує валідний стан навіть після перезавантаження.
- **BOS видимість:** структуральні трикутники отримали більш яскраві кольори (`#4ade80` для BOS, `#facc15` для CHOCH) і товщі лінії (ребра 3px, база 2px), тому їх видно як на 1хв, так і на 5хв барі.
- **Як перевірити:** переключити таймфрейм на `5m`, переконатися, що блок «OHLCV Debug» показує `5m`, графік перезавантажився без мерехтіння, а BOS/CHOCH трикутники лишилися у кадрі. Потім повернутися на `1m` і перевірити, що яскраві кольори не губляться на темному фоні.
