...
| `fxcm:heartbeat` | конектор | Технічний стан стріму (`state`, `last_bar_close_ms`, `ts`). |
| `fxcm:market_status` | конектор | Статус ринку (`open/closed`, `next_open_utc`). |
| `fxcm:price_tik` | конектор | Живі снепшоти bid/ask/mid із OfferTable (cadence орієнтовно кілька секунд; залежить від ринку/конектора). |
| `fxcm:status` | конектор | Агрегований «простий» статус (process/market/price/ohlcv/session/note) для зовнішніх таблиць/моніторингу. |
| `fxcm:commands` | SMC/координатор | Канал команд до конектора (warmup/backfill/set_universe). |
...
Heartbeat — детальна телеметрія про життєвий цикл стріму. У `smc_v1` основним джерелом стану для UI/Stage1 є агрегований канал `fxcm:status`, але heartbeat корисний для глибшої діагностики на стороні конектора.

Конектор публікує heartbeat у форматі `json.dumps(payload, separators=(",", ":"))`, забезпечуючи стабільні назви полів і back-compat.
...
`data/fxcm_status_listener.py` підтримує єдиний стан (агрегація з `fxcm:status`):
...
Канал `fxcm:market_status` зручний для легких консюмерів і для конектора (календар/ETA). У поточній реалізації `smc_v1` для Stage1/UI достатньо `fxcm:status`; за потреби координатор може додатково використовувати `market_status` на своєму боці.

### 2.5 Price stream payload (`fxcm:price_tik`)

Price stream публікує максимум один снапшот на символ за цикл (cadence орієнтовно кілька секунд; може змінюватись) та містить вже нормалізовані bid/ask/mid:
...
### 2.6 Агрегований статус (`fxcm:status`)

`fxcm:status` — канал, який `smc_v1` реально споживає для станів `market/process/price/ohlcv` та сесійних таймерів. Він зведений до простого JSON, щоб UI/Stage1 не мусили інтерпретувати деталі heartbeat/календаря.

Детальний формат див. `docs/fxcm_status_of_conector.md`.

### 2.7 Команди до конектора (`fxcm:commands`)

SMC може публікувати керуючі команди в канал `fxcm:commands`.

Підтримувані типи (контракт конектора):

- `fxcm_warmup` — попросити конектор прогріти історію для `(symbol, tf)`.
- `fxcm_backfill` — попросити підтягнути «хвіст» історії, якщо він застарів.
- `fxcm_set_universe` — попросити змінити universe (список символів/ТF), які конектор стрімить/трекає.

У цьому репо наразі використовується requester S3, який надсилає `fxcm_warmup`/`fxcm_backfill` (див. `docs/uds_smc_update_2025-12-13.md`). Команда `fxcm_set_universe` описана в контракті конектора і може бути використана координатором/оператором, але не є обов’язковою для Stage1/SMC-core.
...
| `FXCM_STATUS_CHANNEL` | Канал агрегованого статусу (типово `fxcm:status`). |
...
1. **Запускаємо конектор** (окреме середовище Python 3.7):
   ...
   Переконатись через `redis-cli SUBSCRIBE fxcm:ohlcv` та `fxcm:status`, що йде активний потік (heartbeat можна підписати додатково для дебагу на стороні конектора).
