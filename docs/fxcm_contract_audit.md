...
- TypedDict контракти + назви каналів (SSOT): `core/contracts/fxcm_channels.py`.
- Soft-валидація payload (SSOT): `core/contracts/fxcm_validate.py`.
- Ключові інваріанти інжесту: `tests/test_fxcm_schema_and_ingestor_contract.py`.
...
- Важливо: підхід `ensure_*` у цьому репо **видалено**. Канонічний OHLCV контракт вимагає `open_time/close_time` у Unix ms (UTC), `close_time >= open_time`, а порядок барів має бути відсортований за `open_time`. Якщо потрібна часова колонка для обчислень, її похідно будують **від `open_time`**, а не з «best-effort» евристик.
