"""Канонічні FXCM telemetry-моделі (pydantic).

Цей модуль описує pydantic-моделі для "telemetry-ish" повідомлень FXCM.
Він існує як SSOT-канон у `core/contracts/*`, щоб у C4.2 можна було
переключити 1 консюмер на імпорт звідси **без** зміни поведінки чи shape.

Важливо:
- Модуль не імпортує нічого з `data/`, `UI/`, `app/` тощо.
- Не додаємо schema_version у FXCM payload зараз (це змінює shape).
"""
...
except Exception:  # pragma: no cover - на випадок іншої версії
...
class FxcmSessionContext(BaseModel):
    """Опис торгової сесії, що додається до heartbeat/market_status/status."""
...
class FxcmHeartbeatContext(BaseModel):
    ...
    seconds_to_open: float | None = Field(
        default=None,
        validation_alias=(
            AliasChoices("seconds_to_open", "next_open_seconds")
            if AliasChoices is not None
            else None
        ),
    )
    ...
    bars_published: int | None = Field(
        default=None,
        validation_alias=(
            AliasChoices("bars_published", "published_bars")
            if AliasChoices is not None
            else None
        ),
    )
...
class FxcmHeartbeat(BaseModel):
    """Heartbeat із Redis-каналу `fxcm:heartbeat` (історично/діагностика)."""
...
class FxcmMarketStatus(BaseModel):
    """Статус ринку з каналу `fxcm:market_status` (історично/діагностика)."""
    ...
    seconds_to_open: float | None = Field(
        default=None,
        validation_alias=(
            AliasChoices("next_open_in_seconds", "seconds_to_open", "next_open_seconds")
            if AliasChoices is not None
            else None
        ),
    )
